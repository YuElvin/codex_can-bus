...
void ethernet_link_check_state(struct netif *netif)
{
  ...
  /* BMSR link status is latched low; read twice to get the current state. */
  (void)HAL_ETH_ReadPHYRegister(&heth, LAN8720_PHY_ADDR, LAN8720_BSR, &phyreg);
  read_status = HAL_ETH_ReadPHYRegister(&heth, LAN8720_PHY_ADDR, LAN8720_BSR, &phyreg);
  ...
  if ((phyreg & LAN8720_BSR_LINK_STATUS) != 0U)
  {
    if (!netif_is_link_up(netif))
    {
