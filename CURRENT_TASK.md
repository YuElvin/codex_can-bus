...
- 早期“FIFO full/lost和RX drop增长”的GDB暂停式读数已确认被调试暂停污染：目标暂停期间外部1 kfps输入会填满硬件FIFO，恢复后会向软件队列突发drain，不能作为正常运行丢帧证据。后续只采用OpenOCD `mdw`非停机读取；高负载P0现已闭合。TF持续写入期间的物理断电、取卡只读恢复仍待现场配合；执行细节见`docs/P0_FAULT_INJECTION.md`。
