...
| W25Q128 | QUADSPI | 已验证 | 默认启动只读 JEDEC ID `EF4018`；`0x00FFF000` 诊断区与 `0x00FFE000` 单规则记录区均已按显式请求验证 |
...
| QSPI W25Q128 | 单规则配置备份、最小 Web/恢复信息、版本信息 | 默认启动只读识别；`0x00FFF000` 固定为诊断区，`0x00FFE000` 为单规则记录区，通用备份必须定义新的演进策略 |
