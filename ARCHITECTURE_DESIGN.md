...
当前 DBC 上传候选文件策略编译基线：FLASH 约 46.97%，RAM_D1 约 20.03%。后续每次引入网络服务、HTTP、DBC 或日志，都要复查 Flash/RAM 水位。
...
3. 已实现 `/www/index.html` 默认页的分块静态读取；DBC 上传当前保存 `/dbc/candidate.dbc` 并返回轻量解析报告，下一步再扩展完整 DBC 配置生效和日志文件接口。
...
当前 HTTP 上传仍是单请求体最小实现：`POST /api/dbc/upload` 只接受 1024 字节以内 text body，先写 `/dbc/upload.write.tmp`，然后把旧候选 `/dbc/candidate.dbc` 备份为 `/dbc/candidate.prev.dbc`，再 rename 新候选；新候选 rename 失败时尝试把旧候选恢复。正式激活预留 `/dbc/active.dbc` 和 `/dbc/active.prev.dbc`：后续 `ConfigTask` 解析候选 `DbcDatabase`，解析成功后再按同样备份/rename 策略切换活动 DBC，并用指针原子切换 `active_dbc`；失败不影响当前活动 DBC。
...
| POST | `/api/dbc/upload` | 上传 DBC | 当前最小实现为 1024 字节以内 text body，保存 `/dbc/candidate.dbc`，旧候选备份 `/dbc/candidate.prev.dbc`，返回 `{ok,data:{candidate,candidateBackup,active,activeBackup,maxBytes,bytes,lines,messages,signals,skipped,errors,valid}}`；后续再扩展 multipart 或分片 |
...
| 9 | TF 静态文件和 DBC 上传 | 部分已验证 | `/www/index.html` 默认静态页可访问；`POST /api/dbc/upload` 可保存 `/dbc/candidate.dbc` 并返回轻量解析报告 |
