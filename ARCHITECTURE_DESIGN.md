...
当前 DBC 上传最小接口编译基线：FLASH 约 46.73%，RAM_D1 约 20.03%。后续每次引入网络服务、HTTP、DBC 或日志，都要复查 Flash/RAM 水位。
...
3. 已实现 `/www/index.html` 默认页的分块静态读取；DBC 上传当前保存 `/dbc/upload.tmp` 并返回轻量解析报告，下一步再扩展完整 DBC 配置生效和日志文件接口。
...
| POST | `/api/dbc/upload` | 上传 DBC | 当前最小实现为 1024 字节以内 text body，保存 `/dbc/upload.tmp` 并返回 `{ok,data:{path,bytes,lines,messages,signals,skipped,errors,valid}}`；后续再扩展 multipart 或分片 |
...
| 9 | TF 静态文件和 DBC 上传 | 部分已验证 | `/www/index.html` 默认静态页可访问；`POST /api/dbc/upload` 可保存 `/dbc/upload.tmp` 并返回轻量解析报告 |
