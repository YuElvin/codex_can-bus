...
| 12 | 稳定性基线 | [进行中；30 分钟子项未通过] | 短时任务/API/CAN 基线已通过；2026-07-13 正式固件 30 分钟插卡静态长跑中 CAN/W5500/API 正常，但默认 LogTask 终态 `last_result=1`、failure/drop=`381/1526`。F-6 已在板级断电上电后捕获干净首错：默认 append `f_open` 的单扇区 `LBA=3826` read 由无错误状态变为 `HAL_SD_ERROR_RX_OVERRUN=0x20`、`DCOUNT=448`、`STA=0x29000`；根因仍待审计。TF 运行中插拔仍为非支持操作 |
