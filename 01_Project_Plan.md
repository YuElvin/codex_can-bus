...
| 12 | 稳定性基线 | [进行中；30 分钟子项未通过] | 短时任务/API/CAN 基线已通过；2026-07-13 正式固件 30 分钟插卡静态长跑中 CAN/W5500/API 正常，但默认 LogTask 终态 `last_result=1`、failure/drop=`381/1526`。F-6 冷启动首错为 `LBA=3826` polling read 的 `HAL_SD_ERROR_RX_OVERRUN=0x20`；F-7 已排除实际 DMA/IDMA、FDCAN2 RX IRQ 和 SDMMC IRQ 处理作为直接机制，下一步仅验证任务切换假设。TF 运行中插拔仍为非支持操作 |
