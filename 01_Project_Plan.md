...
| 12 | 稳定性基线 | [进行中；30 分钟子项未通过] | 短时任务/API/CAN 基线已通过；2026-07-13 正式固件 30 分钟插卡静态长跑中 CAN/W5500/API 正常，但默认 LogTask 终态 `last_result=1`、failure/drop=`381/1526`。重烧录后 35 秒首现同类失败；共享 SD 诊断尚无操作/append 阶段来源，下一步只补充该诊断，不改写入策略；TF 运行中插拔仍为非支持操作 |
