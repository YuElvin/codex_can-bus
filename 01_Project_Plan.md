...
| 7 | FreeRTOS 多任务拆分 | [部分客观已验证] | CAN2 任务已烧录验证为每 50 ms FIFO 接收/外部解码、每 1 s 诊断发送；W5500、状态打印和最小 ConfigTask 独立运行。ConfigTask 已串行完成显式 QSPI 诊断及单规则配置备份，并仅在保存读回成功后触发 RuleTask reload；完整队列/mutex 与通用配置保存待做 |
