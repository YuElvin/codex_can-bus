...
| 7 | FreeRTOS 多任务拆分 | [部分客观已验证] | CAN2、MonitorTask、LogTask、RuleTask、ConfigTask、DbcTask 已独立运行；W5500 状态/HTTP 任务由 W5500 mutex 保护，DBC active 加载与 CAN 解码由 DBC mutex 串行化；DbcTask 仅消费 active reload 窄命令并已实机验证；完整队列、TF 专用任务和通用配置保存仍待做 |
