...
| 12 | 稳定性基线 | [进行中；30 分钟子项未通过] | 短时任务/API/CAN 基线已通过；2026-07-13 正式固件 30 分钟插卡静态长跑中 CAN/W5500/API 正常，但默认 LogTask 终态 `last_result=1`、failure/drop=`381/1526`。F-4 确认首次 append open 读失败为 `HAL_SD_ERROR_RX_OVERRUN=0x20`，BSP 名为 DMA 实际使用轮询 HAL read；下一步只补读请求/寄存器快照，TF 运行中插拔仍为非支持操作 |
