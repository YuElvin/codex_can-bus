...
## 2026-08-09 阶段17 HTTP稳定性门禁重新打开

- 新冷启动网页回归在正常串行队列中再次出现manual响应失败及整机网络失联，因此“网页验证后退出并二次验证”的单次成功不再作为稳定性关闭证据；Classic CAN大DBC功能证据保留，但网页全功能最终验收暂停。
- 当前最小候选只改变W5500断开超时恢复层级：socket0重建成功时不执行芯片复位，失败才fallback；host/构建/反汇编已通过，待物理重启、烧录和实板复测。
- 禁止扩大为多socket、keep-alive、网页自动重试、并发HTTP或业务语义修改；未取得浏览器响应、后续连接、ping和reopen/fallback诊断共同证据前不得关闭门禁。

## 2026-08-09 H后续回归门禁更新

- HTTP失联根因已由W5500 GAR/SIPR损坏及18字节原位恢复实验定位；配置不变量修复候选已通过host、构建、反汇编、烧录和manual安全切换首轮。
- 完成条件尚缺：整板物理冷启动后网页概览/CAN/manual/time/log/rules/candidate功能回归，关闭网页后独立HTTP/ping/CAN二次验证，以及最终治理同步、提交和推送。当前停在需要用户物理断电重上电的`[阻断]`，不改变CAN-FD实板`[未验证]`边界。
