...
| 12 | 稳定性基线 | [进行中；30 分钟子项未通过] | 短时任务/API/CAN 基线已通过；2026-07-13 正式固件 30 分钟插卡静态长跑中 CAN/W5500/API 正常，但默认 LogTask 终态 `last_result=1`、failure/drop=`381/1526`。F-5 已添加读请求/寄存器快照，MCU reset 后立即读到旧连续失败，需板级断电上电捕获干净首次错误；TF 运行中插拔仍为非支持操作 |
