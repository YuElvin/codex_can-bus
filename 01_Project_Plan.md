...
| 12 | 稳定性基线 | [进行中；30 分钟子项未通过] | 短时任务/API/CAN 基线已通过；2026-07-13 正式固件 30 分钟插卡静态长跑中 CAN/W5500/API 正常，但默认 LogTask 终态 `last_result=1`、failure/drop=`381/1526`。F-3 加入来源诊断后，50 秒首现 append stage=`2(open)`、SD operation=`2(read)`、open result=`FR_DISK_ERR`；下一步只审计 SD read 路径，TF 运行中插拔仍为非支持操作 |
