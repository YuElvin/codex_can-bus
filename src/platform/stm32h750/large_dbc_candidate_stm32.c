...
static __attribute__((noinline)) bool publish_manifest(
  ...
  return read_manifest_with_references(g_paths.current, g_manifest_bytes,
...
static void cleanup_owned_formal_files(void) {
   ...
   * If current->previous already happened, the complete old candidate remains
   * recoverable at previous.  Never remove a file not renamed by this call.
   */
