...
| F-008 | DBC 解析和信号缓存 | [部分客观已验证] | 已烧录验证 runtime active DBC 双槽快照、CAN2 轮询调用 portable 解码器和 `SignalCache` 更新；`0x321` TX self-test 得到 2 个缓存项且无解码错误。外部 CANtest RX 解码、实时信号 API、日志、规则和配置生效仍待实现 |
