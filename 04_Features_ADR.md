...
| F-007 | W5500 HTTP/API | [部分客观已验证] | `/api/status`、`/api/can/status`、`/`、`/index.html`、`POST /api/dbc/upload` 已烧录验证；`POST /api/dbc/active` 最小无请求体激活接口已烧录验证 |
| F-008 | DBC 解析和信号缓存 | [部分客观已验证] | HTTP 上传候选文件命名、1024 字节上限和候选替换回滚策略已明确；已烧录验证 portable parser 候选读回解析和活动文件 `/dbc/active.dbc` 最小切换；信号缓存接入和配置生效仍待实现 |
