...
| F-008 | DBC 解析和信号缓存 | [部分客观已验证] | HTTP 上传候选文件命名、1024 字节上限和候选替换回滚策略已明确；已烧录验证 portable parser 候选读回解析、活动文件 `/dbc/active.dbc` 最小切换和运行态 active DBC 双槽快照；CAN 解码接入、信号缓存更新和配置生效仍待实现 |
