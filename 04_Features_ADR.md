...
| F-007 | W5500 HTTP/API | [部分客观已验证] | `/api/status`、`/api/can/status`、`/`、`/index.html`、`POST /api/dbc/upload` 已烧录验证；上传接口当前仅保存 `/dbc/upload.tmp` 并返回轻量解析报告 |
| F-008 | DBC 解析和信号缓存 | [部分客观已验证] | HTTP 上传最小报告已验证；完整 DBC 数据库加载、信号缓存接入和配置生效仍待实现 |
