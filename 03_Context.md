...
| DBC 上传最小接口 | [客观已验证] | `POST /api/dbc/upload` 已烧录验证；当前策略为 1024 字节以内单请求体，经 `/dbc/upload.write.tmp` 写入后带备份替换为 `/dbc/candidate.dbc`，返回候选/活动命名和轻量解析报告；ST-Link 读数 `g_w5500_http_dbc_upload_result=0`、`g_tf_replace_rename_result=0` |
...
- DBC 上传当前只生成候选文件 `/dbc/candidate.dbc`，旧候选保留为 `/dbc/candidate.prev.dbc`；尚未实现 `/dbc/active.dbc` 激活、完整 parser 加载或信号缓存切换。
...
1. 下一步基于 `/dbc/candidate.dbc` 引入可复用 DBC 解析/配置流程：先读取候选文件并用 portable parser 解析，再设计 `/dbc/active.dbc` 激活与失败回滚。
