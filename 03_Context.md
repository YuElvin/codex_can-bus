...
- FreeRTOS 完整多任务架构仍未完成：TF/FatFs、HTTP、配置保存和 DBC 任务尚未拆分，也未引入队列。最小 ConfigTask 仅串行执行既有显式 QSPI 诊断请求，不构成配置保存服务。
- W25Q128 已将 `0x00FFF000` 固定为显式诊断保留区；默认 bring-up 不擦写。显式诊断现由 ConfigTask 串行执行；正式配置备份尚未实现，且不得使用该扇区，后续仍需确定独立地址范围与配置保存机制。
...
5. ConfigTask 当前只拥有既有 W25Q128 诊断请求；后续先确定正式备份地址和数据格式，再引入队列、文件读写或配置保存，不能把本轮诊断串行化表述为配置功能。
