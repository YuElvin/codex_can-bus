...
阶段 C、D、E 已关闭；阶段 F 进行中。F-12 已完成当前固件的30分钟静态插卡日志耐久，原长跑子项现有通过证据。下一派送 F-13 只能先只读审计现有 W5500 链路状态、HTTP socket 状态和网络断开/恢复可观测量，形成固定的最小现场断网恢复验收步骤；不得改源码、烧录、重启、拔插 TF、操作 CANtest 或自行执行断网。
