...
- 当前 HTTP 静态页服务仍是单 socket 最小实现，不支持并发连接、目录映射、HTTP Range、分块传输编码或上传；当前只把 `/` 和 `/index.html` 映射到 `/www/index.html`。
...
1. DBC 上传接口先实现落盘到 `/dbc/*.tmp` 和解析报告，不要一次性加入完整 Web UI。
