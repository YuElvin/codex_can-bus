...
更新时间：2026-07-12（最小 ConfigTask 已接管显式 W25Q128 诊断；默认零擦写与显式擦写读回均已实机验收）
...
| 最小 QSPI 规则配置备份 | [客观已验证] | `0x00FFE000` 独立 4 KiB 扇区保存单规则四个整数参数及 magic/version/checksum；ConfigTask 显式保存、读回匹配、保存成功自动触发 RuleTask reload、非默认参数跨复位恢复和恢复默认值均已 ST-Link 实测 |
...
- W25Q128 已将 `0x00FFF000` 固定为显式诊断保留区，`0x00FFE000` 固定为单规则配置记录区；默认 bring-up 不擦写。后续正式通用配置仍需另行定义记录演进、恢复策略和事务边界，且不得占用诊断区。
- 当前最小 RuleTask、固定 1000 ms 延时、固定高滞回、仅供 ST-Link 验收的手动优先级、单规则 reload 及 QSPI 保存成功后的自动 reload 已现场验证；完整规则文件、HTTP 控制仍未实现，不得将该诊断入口表述为完整规则管理功能。
...
5. ConfigTask 已完成单规则 QSPI 备份及保存成功自动 reload 最小闭环；后续如扩展为通用配置，先定义记录演进、恢复策略与命令来源，再引入队列、文件读写或 HTTP，不能把当前单记录表述为完整配置管理。
