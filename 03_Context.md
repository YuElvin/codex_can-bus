...
| 最小 RuleTask/继电器 | [客观已验证] | 已烧录 50 ms RuleTask：固定 1000 ms 延时、1500 ms 超时安全低、ST-Link 手动 OFF 覆盖优先均已验证；固定高滞回 `on=42434/off=42432` 已实测上阈值置位、中间值 42433 保持、下阈值 42432 释放，PE7/PE8 与锁存诊断一致 |
...
- 当前最小 RuleTask、固定 1000 ms 延时、固定高滞回和仅供 ST-Link 验收的手动优先级已现场验证；完整规则配置、持久化/HTTP 控制仍未实现，不得将该诊断入口表述为完整规则管理功能。
...
4. 后续规则阶段从配置加载选择最小闭环；不把内置 marker 规则和 ST-Link 验收入口直接扩展为未验证的完整 CRUD/API。
