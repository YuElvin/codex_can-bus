...
| 最小 QSPI 规则配置备份 | [客观已验证] | `0x00FFE000` 独立 4 KiB 扇区保存单规则四个整数参数及 magic/version/checksum；ConfigTask 显式保存、读回匹配、非默认参数跨复位恢复和恢复默认值均已 ST-Link 实测 |
...
5. ConfigTask 已完成单规则 QSPI 备份最小闭环；后续如扩展为通用配置，先定义记录演进、恢复策略与命令来源，再引入队列、文件读写或 HTTP，不能把当前单记录表述为完整配置管理。
