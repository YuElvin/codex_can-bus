...
- DBC 上传当前只生成候选文件 `/dbc/candidate.dbc`，旧候选保留为 `/dbc/candidate.prev.dbc`；已接入候选文件读回和 portable parser 报告，但尚未实现 `/dbc/active.dbc` 激活、失败回滚后的活动文件切换或信号缓存切换。
- 本轮为候选解析引入静态 `DbcDatabase` 和 1025 字节候选读回缓冲，最终固件 RAM_D1 占用为 `131680 B / 512 KB = 25.12%`，后续扩展 DBC 数据库上限前必须继续复查内存。
...
1. 下一步设计 `/dbc/active.dbc` 激活与失败回滚：候选解析有效后再用备份/rename 策略切换活动文件，失败必须保持当前活动 DBC 不变。
