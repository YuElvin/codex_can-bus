...
- DBC 上传当前生成候选文件 `/dbc/candidate.dbc`，旧候选保留为 `/dbc/candidate.prev.dbc`；本轮已烧录验证无请求体 `POST /api/dbc/active`，但尚未接入信号缓存切换或配置任务。
- 本轮仍复用单个静态 `DbcDatabase` 和 1025 字节候选读回缓冲，最新固件 RAM_D1 占用为 `131720 B / 512 KB = 25.12%`，后续扩展 DBC 数据库上限前必须继续复查内存。
...
1. 下一步在当前已验证 active 文件基础上，设计运行态 DBC 快照/信号缓存接入；不要直接扩大上传上限、Web UI 或并发 HTTP。
