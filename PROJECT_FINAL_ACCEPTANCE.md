...
| Web 前端 | `/index.html` 为离线单页控制台；一期核心三项为CAN状态/信号严格串行刷新、规则设置、两路继电器手动覆盖 | 浏览器实际访问；独立pcap；页面数据与API JSON对应；规则保存/恢复和继电器操作后的回读/板端GPIO | F-75三项核心已客观通过：303.503 ms首启、29个200/RST=0、规则`42435→42436→42435`、手动序号`1/1→2/2`与ODR`0x100→0x80`；首次手动提交仍暴露HTTP响应交付异常，F-76待修复；DBC浏览器上传/激活本次未复验 |
...
| 故障与稳定性 | 长跑、断网、CAN bus-off/恢复、插卡冷启动后配置/DBC/日志恢复 | 连续计数、错误计数、恢复证据和明确的未通过项 | 进行中；F-12长跑、F-14物理断网恢复、F-17 HTTP优雅关闭修复、F-19冷启动DBC/v3规则/日志恢复、F-25 CAN bus-off 恢复、F-26 CSV 内容均已验证；F-74已取得有效唯一会话分类，但F-75首次手动页面提交再次出现handler 200而浏览器未收到、网络需复位恢复，必须先完成F-76再进入最终全量复验 |
...
| F-76 | 单socket HTTP响应交付异常修复 | 只复现和定位“handler记录200但浏览器Failed to fetch/后续网络暂时不可达”；用单次手动POST的pcap、串口trace和W5500寄存器确定最小故障边界，实施最小修复后重新构建、反汇编、烧录，并验证页面POST响应、后续ping/API及规则/继电器安全态 | 新增Web功能、并发HTTP、扩大API、改变规则/继电器业务语义 |
