...
| 故障与稳定性 | 长跑、断网、CAN bus-off/恢复、插卡冷启动后配置/DBC/日志恢复 | 连续计数、错误计数、恢复证据和明确的未通过项 | 进行中；F-6 已在断电冷启动下确认默认日志 append `f_open` 的单扇区 `LBA=3826` polling read 首错为 RX overrun；根因及其他异常项未完成 |
