...
- TF 静态页、受限 DBC 上传/激活、日志写入及真实恢复路径；
- W25Q128 双槽规则配置持久化；
...
| TF 与日志 | 默认 CSV 路径连续落盘；真实默认读取异常时选择 recovery 并继续写入 | TF 文件大小/内容、LogTask 计数、`mode=1/switch_count=1`、恢复文件增长 | recovery 未完成 |
...
| 故障与稳定性 | 长跑、断网、CAN bus-off/恢复、TF recovery、重启后配置/DBC/日志恢复 | 连续计数、错误计数、恢复证据和明确的未通过项 | 未完成 |
...
| D | LogTask recovery 真实异常覆盖 | 在真实介质条件产生默认路径读取失败时，证明 recovery 选择和写入；不允许人为破坏文件伪造 | 文件轮换、下载、重试策略 |
| E | QSPI diagnostic 失败复核 | 复现并定位 `0xffffffff/erase_count=0` 的根因，或在不影响规则双槽的前提下现场恢复诊断读写 | 把诊断区改成正式配置区 |
