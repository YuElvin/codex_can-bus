...
| 故障与稳定性 | 长跑、断网、CAN bus-off/恢复、插卡冷启动后配置/DBC/日志恢复 | 连续计数、错误计数、恢复证据和明确的未通过项 | 进行中；F-12长跑、F-14物理断网恢复、F-17 HTTP优雅关闭修复、F-19冷启动DBC/v3规则/日志恢复均已验证。RX overrun根因仍未完成；真实CAN bus-off需外部物理位时序错误注入，不能以无ACK/软件伪造替代 |
