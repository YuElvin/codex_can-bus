...
- TF 驻留、离线可用的最小 Web 控制台：展示网关状态、CAN、DBC runtime、实时信号和规则，并通过既有API执行DBC上传/激活与两槽规则管理；
- 现场稳定性、异常和重启恢复证据。

非目标保持不变：不恢复 LAN8720/lwIP 路线，不引入大型前端框架、CDN、鉴权、TLS、WebSocket、后台高频轮询或在线CSV下载，不把 `0x00FFF000` 诊断区作为正式配置区，不把单 socket HTTP 宣称为并发 Web 服务。
...
| 发布完整性 | 工作树干净、分支已推送；所有状态文档与最终固件一致 | `git status`、远端哈希、既有`verify.sh`证据、ELF反汇编、最终烧录记录 | PASS：治理封口提交`fe2154c`已推送；推送后工作树干净，本地与远端ahead/behind=`0/0`，F-76后只有治理Markdown变化且全部证据同源 |
...
- “一个开发步骤”固定为一个可独立现场判定的行为变化：一个阶段、一个因果假设、一个最小运行时边界。只读审计、离线分析和同一假设内的草稿修改不是新的开发步骤，不得因此制造额外烧录或阶段编号。
- 草稿期只做定向静态检查和相关主机测试；准备声明完成时冻结唯一候选，一次完成全量 host CTest、STM32 构建、size、ELF/HEX 哈希、`nm/objdump`。冻结后源码若变化，候选作废并重新生成。
- 每个准备标记“完成”的固件开发步骤仍必须烧录最终候选并做现场闭环；不得把两个独立功能合并为一次烧录。失败候选不提交、不推送，也不扩大目标，只回到同一阶段的最小修复循环。
- 同一候选的启动基线、目标动作、恢复动作、顺序 ping/API、pcap、串口和最终 ST-Link 精确读回合并为一个现场窗口；socket0 HTTP、烧录、halt/read 与网络访问仍必须串行，pcap与串口采集可并行。
- 自动化覆盖构建、反汇编、哈希、OpenOCD残留检查、烧录、浏览器操作、顺序HTTP、pcap/串口离线分析、符号地址解析、读回后的`resume/shutdown`、文档和Git。人工只保留首次抓包`sudo`授权、开发板上下电、TF/网线物理插拔以及无法远程控制的Windows CANtest操作。
- 需要人工时必须一次给出“前置状态→连续操作→每步等待现象→最终回复口令”，不得把一个现场窗口拆成多轮非必要确认。外部条件未满足时标记`BLOCKED`并等待，不重复烧录或试探。
- F-76关闭后直接进入G，不新增功能阶段。G使用同一最终ELF/HEX做“正常联合运行→必要故障注入→发布审计”的合并窗口；历史高成本故障证据只有在最终差异未触及其代码、时钟、任务、mutex或外设配置且最终映像已完成该域烟雾回归时才可复用，否则必须重验。
...
一期功能、现场域和发布完整性均已通过，标记一期全量完成。明确非目标继续作为边界，不是缺口：并发HTTP、TLS/鉴权、WebSocket、在线CSV下载、运行中TF热插拔恢复、无界规则/第三槽、通用配置服务以及恢复LAN8720。F-75现场未重新通过浏览器执行DBC upload/active仅是该次证据边界，既有页面/API能力不是非目标。
...
派送模型按用户最新要求使用 `gpt-5.6-terra`、`high` 推理强度，除非用户再次明确改变；若当前执行环境不能显式选择模型，主会话必须保持同等的固定目标、证据和禁止范围，不得自行扩大阶段。
