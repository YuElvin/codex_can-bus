...
| 故障与稳定性 | 长跑、断网、CAN bus-off/恢复、插卡冷启动后配置/DBC/日志恢复 | 连续计数、错误计数、恢复证据和明确的未通过项 | 进行中；F-6 已确认默认日志 append `f_open` 的单扇区 polling read 首错为 RX overrun。F-11 开启 HWFC 后短时 read/log/CAN/network 正常且 `CLKCR=0x20010`，但 30 分钟耐久、根因及其他异常项仍未完成 |
