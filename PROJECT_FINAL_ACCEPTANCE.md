...
| 故障与稳定性 | 长跑、断网、CAN bus-off/恢复、插卡冷启动后配置/DBC/日志恢复 | 连续计数、错误计数、恢复证据和明确的未通过项 | 进行中；F-6 的 RX overrun 后，F-11 开启 HWFC，F-12 已在当前固件完成30分17秒插卡日志耐久（read/log failure=0、read/write/flush/文件增长、结束 ping/API/CAN 正常）。根因、断网、bus-off与复位恢复仍未完成 |
